# 🔧 Исправление ошибки подключения к БД

## ❌ Проблема:

```
Format of the initialization string does not conform to specification starting at index 0
```

Это означает, что переменная `ConnectionStrings__DefaultConnection` **не читается правильно** или **пустая** при запуске приложения.

**Важно:** Даже если переменная видна в Railway Dashboard, она может не читаться правильно приложением!

---

## ✅ Решение:

### Шаг 1: Открой Railway Dashboard

1. Зайди на https://railway.app
2. Выбери свой проект
3. Нажми на **API сервис** (perceptive-perception)
4. Перейди на вкладку **"Variables"** (или Settings → Variables)

### Шаг 2: Проверь переменные

Убедись что есть переменная:

```
Name: ConnectionStrings__DefaultConnection
Value: ${{Postgres.DATABASE_URL}}
```

**ВАЖНО:**

- Имя переменной должно быть **точно** `ConnectionStrings__DefaultConnection` (с двойным подчеркиванием `__`)
- Значение должно быть `${{Postgres.DATABASE_URL}}` (Railway автоматически подставит connection string)
- **НЕ используй прямой connection string** типа `postgresql://...` - используй только `${{Postgres.DATABASE_URL}}`

### ⚠️ Если переменная установлена напрямую (не через ${{Postgres.DATABASE_URL}}):

1. Удали старую переменную
2. Создай новую с именем `ConnectionStrings__DefaultConnection`
3. В значение вставь: `${{Postgres.DATABASE_URL}}`
4. Сохрани и подожди перезапуска

### Шаг 3: Если переменной нет - добавь её

1. Нажми **"+ New Variable"**
2. В поле **"Name"** введи: `ConnectionStrings__DefaultConnection`
3. В поле **"Value"** введи: `${{Postgres.DATABASE_URL}}`
4. Нажми **"Add"** или **"Save"**

### Шаг 4: Проверь что PostgreSQL сервис запущен

В Railway Dashboard убедись что:

- ✅ **Postgres** сервис имеет статус **"Online"** (зеленый кружок)
- ✅ Если статус не "Online" - нажми на Postgres сервис и проверь логи

### Шаг 5: Перезапусти API сервис

После добавления переменной:

1. Railway автоматически перезапустит API
2. Или вручную: Settings → Restart

### Шаг 6: Проверь что всё работает

Открой в браузере:

```
https://perceptive-perception-production.up.railway.app/health/db
```

Должно показать:

```json
{
  "connectionStringStatus": "✅ SET (length: ...)",
  "canConnect": true,
  "pendingMigrations": [],
  "appliedMigrations": ["20260113173900_InitialCreate"],
  "tablesExist": true
}
```

---

## 🔍 Проверка через Railway Dashboard

### Способ 1: Через Variables

Railway Dashboard → API сервис → Variables

Должна быть переменная:

```
ConnectionStrings__DefaultConnection = ${{Postgres.DATABASE_URL}}
```

### Способ 2: Через Deploy Logs

Railway Dashboard → API сервис → Deploy Logs

Ищи строки:

- ✅ `✅ Database migrations applied successfully` - всё ОК
- ❌ `❌ Cannot connect to database!` - проблема с подключением
- ❌ `ConnectionStrings:DefaultConnection is not configured` - переменная не добавлена

---

## 🎯 Полный список переменных для Railway

Убедись что все эти переменные добавлены:

```env
# Database (КРИТИЧНО!)
ConnectionStrings__DefaultConnection=${{Postgres.DATABASE_URL}}

# OpenAI API Key
OpenAI__ApiKey=sk-proj-твой-ключ

# OpenAI Models
OpenAI__Model=gpt-5-mini
OpenAI__DeepThinkingModel=gpt-5.2
OpenAI__WhisperModel=whisper-1
OpenAI__TTSModel=tts-1
OpenAI__EmbeddingModel=text-embedding-ada-002

# ASP.NET
ASPNETCORE_ENVIRONMENT=Production
ASPNETCORE_URLS=http://0.0.0.0:8080
```

---

## 💡 Частые ошибки:

### ❌ Ошибка 1: Переменная называется неправильно

```
ConnectionStrings_DefaultConnection  ← НЕПРАВИЛЬНО (одно подчеркивание)
ConnectionStrings__DefaultConnection ← ПРАВИЛЬНО (двойное подчеркивание)
```

### ❌ Ошибка 2: Значение неправильное

```
${{Postgres.DATABASE_URL}}  ← ПРАВИЛЬНО (Railway синтаксис)
postgresql://...            ← НЕПРАВИЛЬНО (не используй напрямую)
```

### ❌ Ошибка 3: PostgreSQL сервис не запущен

- Проверь что Postgres сервис имеет статус "Online"
- Если нет - перезапусти его

---

## ✅ После исправления:

1. Railway автоматически перезапустит API
2. Миграции применятся автоматически
3. `/health/db` покажет `canConnect: true`
4. `/api/tasks` будет работать без ошибок

---

**Готово! После добавления переменной всё заработает автоматически!** 🚀
